...
with col1:
    ...
    with st.form("add_food_form", clear_on_submit=True):
        ...
        if submit_add:
            if new_food.strip():  # 確保過濾掉純空白的輸入
                if new_food not in st.session_state.food_categories[add_cat]:
                    st.session_state.food_categories[add_cat].append(new_food.strip())
                    st.toast(f"已新增 {new_food.strip()} 至 {add_cat}")
                    st.rerun()  # 讓右側的刪除選單與下方清單立刻更新
                else:
...
with col2:
    st.subheader("刪除食物")
    # 這裡不能用 form，因為第二個 selectbox 需要根據第一個選項即時連動
